...
if 𝒪[:runserials]
    Random.seed!(𝒪[:seed])
    labels3,ti3 = fit(SplitMergeAlgorithm, X; T=10, ninit=𝒪[:K], benchmark=Val(true))
    println(ti3," ",length(unique(labels3)))
    Random.seed!(𝒪[:seed])
    labels3,ti3 = fit(SplitMergeAlgorithm, X; T=𝒪[:T], ninit=𝒪[:Kinit], benchmark=Val(true))
    println(ti3," ",length(unique(labels3)))
    Random.seed!(𝒪[:seed])
    labels3,ti3 = fit(SplitMergeAlgorithm, X; T=𝒪[:T], ninit=𝒪[:Kinit], benchmark=Val(true))
    println(ti3," ",length(unique(labels3)))
...
Random.seed!(𝒪[:seed])
labelsp3,pti3 = fit(SplitMergeAlgorithm, X; T=10, ninit=𝒪[:K], benchmark=Val(true), ncpu=𝒪[:ncpu])
println(pti3," ", length(unique(labelsp3)))
Random.seed!(𝒪[:seed])
labelsp3,pti3 = fit(SplitMergeAlgorithm, X; T=𝒪[:T], ninit=𝒪[:Kinit], benchmark=Val(true), ncpu=𝒪[:ncpu])
println(pti3," ", length(unique(labelsp3)))
Random.seed!(𝒪[:seed])
labelsp3,pti3 = fit(SplitMergeAlgorithm, X; T=𝒪[:T], ninit=𝒪[:Kinit], benchmark=Val(true), ncpu=𝒪[:ncpu])
println(pti3," ", length(unique(labelsp3)))
